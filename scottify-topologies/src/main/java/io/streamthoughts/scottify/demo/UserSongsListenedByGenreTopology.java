...
import io.streamthoughts.scottify.demo.model.UserSongsListenedByGenre;
...
@Component
@javax.inject.Named("UserSongsListenedByGenreTopology")
@TopologyInfo(description = "Scottify - Aggregate for each user the total number of listened songs by genre")
public class UserSongsListenedByGenreTopology extends EventStreamSupport implements TopologyProvider {
    ...
    public Topology topology() {
        ...
        // Joins SongListenedEvent with Users and Albums.
        KGroupedStream<String, Tuple<User, Album>> groupedStreams = songListenedEvents
            ...
            .groupByKey(Grouped.as("group-by-username"));

        // Finally, per user, count the number of musics listen per genre.
        KTable<String, UserSongsListenedByGenre> count = groupedStreams.aggregate(
                UserSongsListenedByGenre::new,  /* initializer */
            (userId, tuple, aggregate) -> aggregate.update(tuple.left(), tuple.right()), /* aggregator */
            Named.as("count-user-listened-songs-by-genre"),
            Materialized.<String, UserSongsListenedByGenre, KeyValueStore<Bytes, byte[]> /* materialized store */>
                    as("UserListenedSongsByGenre")
                    .withValueSerde(newJsonSerde(UserSongsListenedByGenre.class))
        );

        count.toStream()
             .to("agg-listened-genres-by-user", Produced.with(Serdes.String(), newJsonSerde(UserSongsListenedByGenre.class)));

        // Example to use Server-Sent-Event

        // 1) register a new event-stream type named 'listened'.
        final String eventType = "listened";
        addEventStream(
            new EventStream.Builder(eventType)
                .withMaxBlockingTime(Duration.ofSeconds(5))
                .withQueueSize(10_000)
                .withQueueLimitHandler(LimitHandlers.dropHeadOnLimitReached())
                .build()
        );
        // 2) send message to event-stream.
        count
            .toStream()
            .foreach( (k, v) -> send(eventType, k, v), Named.as("server-sent-event"));
